...
%
    ...
    good_red(GR1),
	subtract(GR1,NoRed,YesR),
	subtract(GR1,[1,33],XRed),
	numlist(1,16,B), subtract(B,NoBlue,YesB),	
%	
	set_random(seed(random)),
	pick_nums(Count,YesB,OkB),
	pick_red(Count, XRed, YesR, OkB, Res), length(Res,Count), !,
	maplist(writeln,Res),
	ssqNumF(F),
    tell(F), maplist(format('~d ~d ~d ~d ~d ~d ~d~n'), Res), told.

pick_red(1, XRed, _, OkB, [H|_]) :-
    pick_nums(6,XRed,R1), sort(R1,Red), nth1(1,OkB,Blue),
	append(Red,[Blue],H), !.
pick_red(Count, XRed, YesR, OkB, [H|T]) :-
	pick_nums(6,YesR,R1), sort(R1,Red), nth1(Count,OkB,Blue),
	append(Red,[Blue],H), C1 is Count-1,
	pick_red(C1,XRed,YesR,OkB,T).

pick_nums(0, _, []).
pick_nums(Count, From, [X|SelectedFromRemaining]) :-
  random_member(X, From), sleep(0.3),
