...
(define (win-ssq count noRed noBlue)
  (let ((gRed (good-red))
		(yesRed (set-difference gRed (str2lst noRed)))
		(okBlue '()) (num '()) (red1 '()))
	(seed->random-state (current-time))
	(set! okBlue (pickNums count (set-difference *BLUE-NUMS* (str2lst noBlue))))
	(call-with-output-file +ssq-num+
	  (lambda(h)
		(map (lambda(n)
			   (if (= n (- count 1))
				   (set! red1 (pickNums 6 gRed))
