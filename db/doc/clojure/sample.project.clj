...
;; descriptions as below).
    :mailing-list {:name "sample mailing list"
                   ...
                   :other-archives ["http://example.org/sample-list-archive2" "http://example.org/sample-list-archive3"]
...
;; the following to share code from the test suite:
    :checkout-deps-shares [:source-path :test-path ~(fn [p] (str (:root p) "/lib/dev/*"))]
...
;; so in effect this simply omits Clojars.
    ...
    :repositories {"java.net" "http://download.java.net/maven/2" "sonatype"
