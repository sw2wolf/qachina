...
hit_sum(HitNo, No) :-
	...
	append(RedN,[B2],No), !,
	intersection(RedN, RedH, X), length(X,HitR),
	hit_desc(HitR,HitB,Desc),
...
qachina :-
	thread_create(shell("cd /media/D/www/qachina; ./start.bat"),  _,
	    [alias(http_server_monitor), detached(true)]).
...
sys_info :-
	...
	format('SWI-Prolog/~w.~w.~w', [Major, Minor, Patch]).
