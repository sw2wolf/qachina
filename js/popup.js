...
function InnerConfig(){
	//进入JS赋值：公告时间、用户数、主题数、帖子数、今日贴、昨日贴、最高发贴、最高发贴日
	this.Index_Top = ["iAnnTime","iUserNum","iTopicNum","iPostNum","iTodayNum","iYesTodayNum","iMaxPost","iMaxPostDate"];
	this.Index_Top_Value = [""];
	//进入JS赋值：用户IP、系统、浏览器、显示详细列表字样、总在线、用户在线、客人在线、最大在线、最大在线时间、论坛建立时间
	this.Index_Footer = ["ifIP","ifSys","ifBrw","ifOnlineStr","ifOnlineNum","ifUserNum","ifGuestNum","ifMaxNum","ifMaxDate","ifBuildDate"];
	this.Index_Footer_Value = [""];
}
//论坛跳转下拉菜单部分,by Lao_Mi
function BoardJumpList(boardid){
		...
		if(boardid!=0)
		{
			...
			{
				LoadBoard = nodelist1[i].getAttribute("boardid");
...
GetBoardXml("index.asp?action=xml");